...
impl GraphExecutor {
    ...
    pub fn instantiate(&mut self, wasm_graph_file: String) -> Result<()> {
        ...
        // will have access to.
        let dir = unsafe { Dir::open_ambient_dir(".") }?;
        // let dir = unsafe { Dir::from_std_file(f_dir) };

        println!("DEBUG: {:?}", dir);
        let mut builder = WasiCtxBuilder::new().inherit_stdio().inherit_args()?;
        builder = builder.preopened_dir(dir, ".")?;

        // println!("DEBUG: {:?}", builder);
